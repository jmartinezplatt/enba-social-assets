...
const PAGE_TOKEN = execSync('powershell -Command "[System.Environment]::GetEnvironmentVariable(\'META_ACCESS_TOKEN\',\'User\')"', {encoding:'utf8'}).trim();
...
// 1. Crear credencial con META_ACCESS_TOKEN (Page Token)
console.log('Creando credencial Meta API ENBA - Page Token...');
const cred = await n8n('POST', '/credentials', {
  name: 'Meta API ENBA - Page Token',
  type: 'httpHeaderAuth',
  data: {
    name: 'Authorization',
    value: `Bearer ${PAGE_TOKEN}`
  }
});
const CRED_ID = cred.id;
console.log('Credencial creada. ID:', CRED_ID);
...
// 3. PUT workflow con nueva credencial
const updatedWorkflow = {
  ...
  nodes: [
    ...
    },
    {
      ...
      typeVersion: 4.2,
      position: [1080, 420],
      credentials: { httpHeaderAuth: { id: CRED_ID, name: 'Meta API ENBA - Page Token' } },
      parameters: {
        method: 'POST',
...
console.log('Aplicando PUT al workflow con nueva credencial...');
...
const credEnContainer = wf.nodes?.find(n => n.name === 'IG Create Container')?.credentials?.httpHeaderAuth;
console.log('Active:', wf.active);
console.log('Credencial en IG Create Container:', JSON.stringify(credEnContainer));
