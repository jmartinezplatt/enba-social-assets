...
const inputVideo = path.join(baseDir, "media", "5c04e430-97c1-42ee-980c-5cfac81e6d66.mp4");
const logoPath = path.join(baseDir, "brand", "ENBA-horizontal-oscuro.svg");
const outputDir = path.join(baseDir, "final");
const assetDir = path.join(outputDir, "tt03-endcard-v1");

const outputVideo = path.join(outputDir, "TT03-final-enba-v1-endcard-logo-fade.mp4");
const mainClip = path.join(assetDir, "tt03-main-cut-12_8s.mp4");
const endCardPng = path.join(assetDir, "tt03-endcard-logo-only.png");
const endCardVideo = path.join(assetDir, "tt03-endcard-logo-fade.mp4");

const textOverlays = [
  {
    file: path.join(assetDir, "tt03-text-01.png"),
    lines: ["Buenos Aires"]
  },
  {
    file: path.join(assetDir, "tt03-text-02.png"),
    lines: ["Buenos Aires", "volvemos a encontrarnos"]
  },
  {
    file: path.join(assetDir, "tt03-text-03.png"),
    lines: ["Buenos Aires", "volvemos a encontrarnos", "pero en el río"]
  }
];
...
function buildMainClip() {
  const filter = [
    ...
    "[base][1:v]overlay=0:0:enable='between(t,0.25,2.00)'[v1]",
    "[v1][2:v]overlay=0:0:enable='between(t,2.00,4.20)'[v2]",
    "[v2][3:v]overlay=0:0:enable='gte(t,4.20)'[v3]"
  ].join(";");
