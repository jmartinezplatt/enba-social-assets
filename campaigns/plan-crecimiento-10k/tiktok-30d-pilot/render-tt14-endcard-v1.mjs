...
const inputVideo = path.join(baseDir, "media", "b6c0e560-9ba5-42eb-8ab2-30fdcebdbb2e.mp4");
const logoPath = path.join(baseDir, "brand", "ENBA-horizontal-oscuro.svg");
const outputDir = path.join(baseDir, "final");
const assetDir = path.join(outputDir, "tt14-endcard-v1");

const outputVideo = path.join(outputDir, "TT14-final-enba-v1-endcard-logo-fade.mp4");
const mainClip = path.join(assetDir, "tt14-main-cut-10_3s.mp4");
const endCardPng = path.join(assetDir, "tt14-endcard-logo-only.png");
const endCardVideo = path.join(assetDir, "tt14-endcard-logo-fade.mp4");

const textOverlays = [
  { file: path.join(assetDir, "tt14-text-01.png"), lines: ["No es solo un plan"] },
  { file: path.join(assetDir, "tt14-text-02.png"), lines: ["No es solo un plan", "es otra velocidad"] },
  { file: path.join(assetDir, "tt14-text-03.png"), lines: ["No es solo un plan", "es otra velocidad", "otra cabeza"] }
];
...
<head>
  ...
  <style>
    ...
    .stack { position: absolute; left: 64px; bottom: 650px; display: flex; flex-direction: column; gap: 14px; width: 860px; }
    ...
    .line-2 span { font-size: 66px; }
    .line-3 span { font-size: 62px; }
    .endcard {
...
function buildMainClip() {
  const filter = [
    ...
    "[base][1:v]overlay=0:0:enable='between(t,0.20,1.80)'[v1]",
    "[v1][2:v]overlay=0:0:enable='between(t,1.80,3.80)'[v2]",
    "[v2][3:v]overlay=0:0:enable='gte(t,3.80)'[v3]"
  ].join(";");
