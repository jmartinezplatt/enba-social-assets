...
const inputVideo = path.join(baseDir, "media", "IMG_1556.MP4");
const logoPath = path.join(baseDir, "brand", "ENBA-horizontal-oscuro.svg");
const outputDir = path.join(baseDir, "final");
const assetDir = path.join(outputDir, "tt11-endcard-v1");

const outputVideo = path.join(outputDir, "TT11-final-enba-v1-endcard-logo-fade.mp4");
const mainClip = path.join(assetDir, "tt11-main-looped-7_6s.mp4");
const endCardPng = path.join(assetDir, "tt11-endcard-logo-only.png");
const endCardVideo = path.join(assetDir, "tt11-endcard-logo-fade.mp4");

const textOverlays = [
  { file: path.join(assetDir, "tt11-text-01.png"), lines: ["Si tu finde siempre se ve igual"] },
  { file: path.join(assetDir, "tt11-text-02.png"), lines: ["Si tu finde siempre se ve igual", "tal vez el problema"] },
  { file: path.join(assetDir, "tt11-text-03.png"), lines: ["Si tu finde siempre se ve igual", "tal vez el problema", "es el plan"] }
];
...
<head>
  ...
  <style>
    ...
    .stack { position: absolute; left: 64px; bottom: 640px; display: flex; flex-direction: column; gap: 14px; width: 860px; }
    ...
    .line-1 span { font-size: 60px; }
    .line-2 span, .line-3 span { font-size: 66px; }
    .endcard {
...
function buildEndCardHtml(logoSvg) {
  return htmlShell(`<div class="endcard"><div class="endcard-logo">${logoSvg}</div></div>`);
}
...
async function renderAssets() {
  ...
  try {
    for (const overlay of textOverlays) await screenshotHtml(browser, buildTextHtml(overlay.lines), overlay.file);
...
function buildMainClip() {
  const filter = [
    "[0:v]scale=1080:1920:force_original_aspect_ratio=increase,crop=1080:1920,setpts=2.6*PTS[base]",
    "[base][1:v]overlay=0:0:enable='between(t,0.20,2.20)'[v1]",
    "[v1][2:v]overlay=0:0:enable='between(t,2.20,4.80)'[v2]",
    "[v2][3:v]overlay=0:0:enable='gte(t,4.80)'[v3]"
  ].join(";");

  run("ffmpeg", [
    "-y", "-stream_loop", "2", "-i", inputVideo,
    "-i", textOverlays[0].file, "-i", textOverlays[1].file, "-i", textOverlays[2].file,
