...
const inputVideo = path.join(baseDir, "media", "IMG_5615.MOV");
const logoPath = path.join(baseDir, "brand", "ENBA-horizontal-oscuro.svg");
const outputDir = path.join(baseDir, "final");
const assetDir = path.join(outputDir, "tt02-endcard-v1");

const outputVideo = path.join(outputDir, "TT02-final-enba-v1-endcard-logo-fade.mp4");
const mainClip = path.join(assetDir, "tt02-main-cut-15_8s.mp4");
const endCardPng = path.join(assetDir, "tt02-endcard-logo-only.png");
const endCardVideo = path.join(assetDir, "tt02-endcard-logo-fade.mp4");

const textOverlays = [
  { file: path.join(assetDir, "tt02-text-01.png"), lines: ["Tu primera vez"] },
  { file: path.join(assetDir, "tt02-text-02.png"), lines: ["Tu primera vez", "en un velero"] },
  { file: path.join(assetDir, "tt02-text-03.png"), lines: ["Tu primera vez", "en un velero", "puede empezar acá"] }
];
...
<head>
  ...
  <style>
    ...
    .stack { position: absolute; left: 64px; bottom: 630px; display: flex; flex-direction: column; gap: 14px; width: 860px; }
...
function buildMainClip() {
  const filter = [
    ...
    "[base][1:v]overlay=0:0:enable='between(t,0.25,2.40)'[v1]",
    "[v1][2:v]overlay=0:0:enable='between(t,2.40,5.40)'[v2]",
    "[v2][3:v]overlay=0:0:enable='gte(t,5.40)'[v3]"
  ].join(";");
